...
from modules.query_and_export import properties_simple, \
    properties_simple_locaux, properties_to_csv, properties_and_history, \
    past_properties, properties_and_history_locaux, past_properties_locaux
...
def display_export(conn, sirens, properties_function, locaux_function):
    ...
    # Traitement des résultats pour les parcelles
    process_export_result(result_properties, "parcelles")
    # Traitement des résultats pour les locaux
    process_export_result(result_locaux, "locaux")


def process_export_result(result, suffix):
    if isinstance(result, str):
        ...
        if properties:
            ...
            if export_choice == 'oui':
                ...
                properties_to_csv(properties, column_names, filename=f"{filename}-{suffix}.csv")
        else:
    ...
    while True:
        ...
        if choice == 'init':
            ...
            with sqlite3.connect('cadastral_data.db') as conn:
                if choice == '2':
                    display_export(conn, sirens, properties_simple, properties_simple_locaux)
                elif choice == '3':
                    display_export(conn, sirens, properties_and_history, properties_and_history_locaux)
                elif choice == '4':
                    display_export(conn, sirens, past_properties, past_properties_locaux)
        elif choice == 'q':
